...
export function ProductCatalogFeature({
  ...
  const quickCategories = (quickCategoriesFromBackend?.length
    ? quickCategoriesFromBackend
    : catalog.categoryOptions
        .filter((option): option is { value: ProductCategory; label: string } => option.value !== 'ALL')
        .map((option) => ({
  ...
  return (
    <ProductCatalog
      ...
      subcategories={subcategories.map((subcategory) => ({
        ...
        href: selectedSubcategorySlug === subcategory.slug ? '#' : `?subcategory=${subcategory.slug}`,
      ...
      onPreviousHero={() =>
        setActiveHeroIndex((current) => (current - 1 + safeHeroSlides.length) % safeHeroSlides.length)
      }
